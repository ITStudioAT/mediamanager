<template>
    ...
    <v-container fluid class="ma-0 w-100 pa-2">
        <v-card flat tile variant="outlined">
            <v-card-text>
                <v-card flat tile>
                    <v-card-text>
                        <!-- MENÜ -->
                        <v-row class="d-flex flex-row ga-2 mt-0" no-gutters>
                            <v-col cols="12" class="d-flex flex-row flex-wrap align-center ga-2">
                                <its-menu-button color="mm-bg-secondary" title="Auswahl"
                                    icon="mdi-image-size-select-actual" @click="select" />
                            </v-col>
                        </v-row>
                    </v-card-text>
                </v-card>

                <v-card flat tile color="mm-primary">
                    <v-card-text>
                        <!-- FILEUPLOAD -->
                        ...
                        <v-row no-gutters class="my-4" v-if="is_loading == 0">
                            <v-col class="d-flex flex-row flex-wrap align-center ga-2">
                                <!-- Alle selektieren -->
                                <v-btn color="secondary" v-if="selected_files.length < files.length"
                                    @click="onSelectAll"><v-icon icon="mdi-select-all" /></v-btn>
                                <!-- Alle abwählen -->
                                ...
                                <!-- Löschen durchführen -->
                                <v-btn color="error" v-if="selected_files.length > 0 && delete_level == 1"
                                    @click="destroyFiles"><v-icon icon="mdi-delete" /></v-btn>
                ...
                <v-card flat tile color="mm-primary mt-4">
                    <v-card-text>
                        <v-row class="my-4">
                            <!-- FILE-LIST -->
                            ...
                            <v-col>
                                <v-card tile flat :loading="is_loading_preview > 0">
                                    <v-card-text class="d-flex flex-row align-center justify-center"
                                        style="min-height: 300px;" v-if="is_loading_preview > 0">
