...
class MediamanagerController extends Controller
{
    ...
    public function uploadPatch(Request $request)
    {
        ...
        return response('', 200)
            ->header('Upload-Offset', trim(str_replace(["\r", "\n"], '', (string) $newSize)));
    }
    ...
    public function uploadGet(Request $request)
    {
        ...
        $mediamanagerService = new MediamanagerService();
        $offset = $mediamanagerService->uploadGet($request, $path);   // <-- define it!
    ...
    public function folderStructure(Request $request)
    {
        ...
        $path = $base_path ? public_path($base_path) : public_path(config('mediamanager.path'));
    ...
    public function download(Request $request)
    {
        ...
        return response()->download(public_path($file), basename($file));
    }
    ...
    public function saveFilename(SaveFilenameRequest $request)
    {
        ...
        $mediamanagerService = new MediamanagerService();
        $mediamanagerService->rename(
            $validated['path'],
            $validated['path'],
            $validated['current_filename'],
            $validated['filename'] . '.' . $validated['extension']
        );
